...
-- ╔══════════════════════════════════════════════════════════╗
-- ║  17. MULTI-TENANT SHARED NUMBER ARCHITECTURE            ║
-- ╚══════════════════════════════════════════════════════════╝
-- Run this section to enable shared WhatsApp number routing.
-- All existing data is preserved — new columns default safely.

-- Business category (shown in business picker, e.g. "Food & Beverage")
ALTER TABLE businesses ADD COLUMN IF NOT EXISTS category TEXT;

-- Display order in the business picker (lower = shown first)
ALTER TABLE businesses ADD COLUMN IF NOT EXISTS display_order INTEGER DEFAULT 999;

-- Whether this business uses the shared platform number
-- (true = no own WhatsApp setup needed)
ALTER TABLE businesses ADD COLUMN IF NOT EXISTS use_shared_number BOOLEAN DEFAULT true;

-- Business owner's contact/support phone (different from WhatsApp API number)
ALTER TABLE businesses ADD COLUMN IF NOT EXISTS contact_phone TEXT;

-- Platform-level carts row uses business_id=0. Ensure no FK constraint blocks it:
-- (If your carts table has a FK to businesses.id, run this first)
-- ALTER TABLE carts DROP CONSTRAINT IF EXISTS carts_business_id_fkey;

-- Indexes
CREATE INDEX IF NOT EXISTS ix_businesses_shared
  ON businesses (use_shared_number, is_active)
  WHERE use_shared_number = true;

CREATE INDEX IF NOT EXISTS ix_businesses_display
  ON businesses (display_order NULLS LAST, id);
...
WHERE table_name = 'businesses'
  AND column_name IN (
    'category','display_order','use_shared_number','contact_phone',
    'is_active','ecocash_number','paypal_email'
  )
ORDER BY column_name;


-- ╔══════════════════════════════════════════════════════════╗
-- ║  18. AGENT vs AI MESSAGE DIFFERENTIATION                ║
-- ╚══════════════════════════════════════════════════════════╝
-- Adds columns to messages table so inbox can show different
-- visual styling for AI-generated vs human-agent responses.

-- Who sent this message: 'ai' | 'agent' | null (customer)
ALTER TABLE messages ADD COLUMN IF NOT EXISTS sender_type TEXT;

-- Human agent's username (shown as badge on their messages)
ALTER TABLE messages ADD COLUMN IF NOT EXISTS sender_name TEXT;

-- Index for filtering agent messages in analytics
CREATE INDEX IF NOT EXISTS ix_messages_sender_type
  ON messages (sender_type, business_id)
  WHERE sender_type IS NOT NULL;

-- Backfill: existing outgoing messages are assumed to be AI
UPDATE messages
  SET sender_type = 'ai'
...
WHERE table_name = 'messages'
  AND column_name IN ('sender_type', 'sender_name', 'direction', 'wa_message_id')
ORDER BY column_name;
